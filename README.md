...
İkinci sprintte, Sprint 1 retrospektifinde kararlaştırıldığı üzere kullanıcı işlemleri, ürün yönetimi ve yapay zekâ entegrasyonu geliştirmelerine odaklanılmıştır. Bu bölüm, "Backend + AI + Bildirim" görev grubu kapsamında tamamlanan işleri yansıtır; ekibin diğer görev gruplarına ait maddeler ilgili takım üyeleri tarafından ayrıca eklenecektir.

## Ürün Durumu

Sprint 2 kapsamında "Backend + AI + Bildirim" görevleri tamamlanmıştır:

- `frontend/src/services/ai.js` oluşturuldu — Google Gemini API ile tarif önerisi servisi.
  - Malzeme listesinden 3 tarif üreten temel prompt (JSON formatında: ad, malzeme, süre, kalori, adımlar).
  ...
  - AI yanıtını doğrulayan parse fonksiyonu ve hata yönetimi (timeout, boş yanıt, API limiti, parse hatası, ağ hatası).
...
Sprint 1 retrospektifinde belirlenen yapay zekâ entegrasyonu hedefi doğrultusunda, buzdolabındaki malzemelerden AI destekli tarif önerisi üreten servis ve son kullanma tarihi yaklaşan ürünler için e-posta bildirimi altyapısı tamamlanmıştır. Servisler, ileride UI'a bağlanabilecek şekilde bağımsız fonksiyonlar olarak tasarlanmış ve handoff dokümanıyla teslim edilmiştir.
...
- Üçüncü sprintte, bu servislerin ilgili sayfalara (Recipes.jsx, Fridge.jsx) bağlanmasına odaklanılması önerilmektedir.
